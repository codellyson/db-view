...
| 5 ◐ | Import/export + saved connections in OS keychain | `saved-connections` (full GET/POST/DELETE/PATCH on OS keychain), `import` (CSV → batched bulk INSERT in tx); `upload-sqlite` deferred (needs rusqlite + provider abstraction) | medium — filesystem semantics differ, keychain is platform-specific |
| 6 ◐ | Polish + ship | release profile (LTO, panic=abort, strip); code signing + auto-update + workspace split outstanding | medium — code signing, auto-update, bundle size |
...
| 2026-05-24 | `field.source` resolved in `db_run_query` via two pg_catalog lookups | `tokio_postgres::Column` exposes `table_oid()` + `column_id()` straight from the prepared statement's row description; we widen `ColumnMeta` to carry them (plus the type's OID for `dataTypeID`) and the SQL editor's FK-navigator on result rows now activates. OIDs are integers we control, so inlining them in the `IN (…)` clause is safe. No caching across queries — the two extra round-trips are cheap, and skipping the cache avoids touching `PgConnection`'s lock surface. |
| 2026-05-24 | `/api/import` ported; `/api/upload-sqlite` deferred | CSV import was a clean port — the UI already POSTs `{ schema, table, columns, rows }` JSON (parsing happens in the dialog component), so `db_import` is a batched bulk INSERT inside our existing `run_transaction`. SQLite upload is *not* a route port — it needs `rusqlite`, a provider abstraction, and a Tauri dialog plugin for native file picking, which together are multi-hour infra work outside Phase 5's scope. Defer until either a user asks or the codebase needs a second DB engine. |
| 2026-05-24 | Phase 6 release profile in Cargo.toml | `lto=true` + `codegen-units=1` for whole-program LTO; `panic="abort"` removes unwind tables (smaller binary, matches Tauri's recommended profile); `strip=true` drops debug symbols from the shipped binary. Dev builds are untouched. Code signing (~$99/yr Apple Dev) and auto-update (Tauri Updater plugin + signed manifest hosting) still need account/infra setup outside the codebase. Workspace split deferred — invasive with no immediate functional gain. |
